...
const LIMITS = {
  ...
  smokeStir: [0, 4, 'smoke cursor stir'], wakeForce: [0, 2, 'skulls part the smoke'], smokeDecay: [0.05, 3, 'smoke stir fade rate'], skullSize: [0.5, 1.8, 'hero skull size'], swarmSizeMin: [0.05, 2, 'flood skull smallest'], swarmSizeMax: [0.05, 2, 'flood skull largest'], breatheRate: [2, 40, 'breathing pace (per min)'],
    ...
    try {
      ...
      if (request.method === 'POST' && path === '/api/chat') {
        ...
        const system = [
          ...
          'LEVELS: musicVolume, sfxVolume, gritoVolume and voiceVolume are 0..1. "Turn',
          'yourself down" / "speak quieter" is voiceVolume; "quieter music" is',
          'musicVolume; "quieter gritos" is gritoVolume. Halve the current value for',
          '"quieter", set 1 for "full", never go to 0 for a volume request (that is a',
          'mute, which has its own switch).',
